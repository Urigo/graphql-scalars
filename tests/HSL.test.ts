...
import { GraphQLHSL } from '../src/scalars/HSL';

describe(`HSL`, () => {
  describe(`valid`, () => {
    it(`serialize`, () => {
      expect(GraphQLHSL.serialize(`hsl(270, 60%, 70%)`)).toEqual(
        `hsl(270, 60%, 70%)`,
      );
    });

    it(`parseValue`, () => {
      expect(GraphQLHSL.parseValue(`hsl(270, 60%, 70%)`)).toEqual(
        `hsl(270, 60%, 70%)`,
      );
    });
  ...
  describe(`invalid`, () => {
    describe(`not a valid HSL color`, () => {
      ...
      it(`parseLiteral`, () => {
        expect(() =>
          GraphQLHSL.parseLiteral({ value: 123, kind: Kind.INT } as any, {}),
