...
import { GraphQLHSLA } from '../src/scalars/HSLA';
  ...
  describe(`invalid`, () => {
    describe(`not a valid HSLA color`, () => {
      it(`serialize`, () => {
        ...
        expect(() =>
          GraphQLHSLA.serialize(`this is not an hsla color`),
      ...
      it(`parseValue`, () => {
        expect(() => GraphQLHSLA.serialize(123)).toThrow(/Value is not string/);
        expect(() =>
          GraphQLHSLA.parseValue(`this is not an hsla color`),
        ).toThrow(/Value is not a valid HSLA color/);
      });

      it(`parseLiteral`, () => {
        expect(() =>
          GraphQLHSLA.parseLiteral({ value: 123, kind: Kind.INT } as any, {}),
