...
import { VALID_LOCAL_TIMES, INVALID_LOCAL_TIMES } from './LocalTime.test.js';
    ...
    describe(`not a valid LocalTime`, () => {
      it(`serialize`, () => {
        ...
        INVALID_LOCAL_END_TIMES.forEach(testValue => {
          expect(() => GraphQLLocalEndTime.serialize(testValue)).toThrow(/Value is not a valid LocalTime/);
        });
      });
      ...
      it(`parseValue`, () => {
        ...
        INVALID_LOCAL_END_TIMES.forEach(testValue => {
          expect(() => GraphQLLocalEndTime.parseValue(testValue)).toThrow(/Value is not a valid LocalTime/);
        });
      });

      it(`parseLiteral`, () => {
        expect(() => GraphQLLocalEndTime.parseLiteral({ value: 123, kind: Kind.INT } as any, {})).toThrow(
          /Can only validate strings as local times but got a/
        );

        expect(() => GraphQLLocalEndTime.parseLiteral({ value: false, kind: Kind.BOOLEAN }, {})).toThrow(
          /Can only validate strings as local times but got a/
        );
        INVALID_LOCAL_END_TIMES.forEach(testValue => {
          expect(() => GraphQLLocalEndTime.parseLiteral({ value: testValue, kind: Kind.STRING }, {})).toThrow(
