    ...
    describe(`not a valid LocalDate`, () => {
      it(`serialize`, () => {
        ...
        INVALID_LOCAL_DATES.forEach(testValue => {
          expect(() => GraphQLLocalDate.serialize(testValue)).toThrow(/Value is not a valid LocalDate/);
        });
      });
      ...
      it(`parseLiteral`, () => {
        expect(() => GraphQLLocalDate.parseLiteral({ value: 123, kind: Kind.INT }, {})).toThrow(
          /Can only validate strings as local dates but got a/
        );

        expect(() => GraphQLLocalDate.parseLiteral({ value: false, kind: Kind.BOOLEAN }, {})).toThrow(
          /Can only validate strings as local dates but got a/
        );
        INVALID_LOCAL_DATES.forEach(testValue => {
          expect(() => GraphQLLocalDate.parseLiteral({ value: testValue, kind: Kind.STRING }, {})).toThrow(
