...
A time string at UTC, such as `10:15:30Z`, is compliant with the `full-time` format outlined in section 5.6 of the [RFC 3339](./rfc3339.txt) profile of the ISO 8601 standard for representation of dates and times using the Gregorian calendar.
...
Javascript Date instances are coerced to an RFC 3339 compliant time string by extracting the UTC part. Invalid Date instances raise a fielding error.
