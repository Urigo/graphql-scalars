...
<Callout>
  Based on [Graphql-bigint](https://github.com/stems/graphql-bigint). Even
  though `stems`'s implementation looks the same as the `BigInt` scalar, it is
  different because our `BigInt` implementation uses JavaScript's native
  `BigInt` type while `SafeInt` implementation doesn't support more than 52-bit
  integers.
</Callout>
